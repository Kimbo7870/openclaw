  ...
  addGatewayClientOptions(
    cron
      ...
      .action(async (opts: GatewayRpcOpts & Record<string, unknown>, cmd?: Command) => {
        try {
          const sessionExplicitlySet =
            typeof cmd?.getOptionValueSource === "function" &&
            cmd.getOptionValueSource("session") === "cli";
          const params = buildCronAddParams(opts, { sessionExplicitlySet });
...
export function buildCronAddParams(
  opts: Record<string, unknown>,
  ctx: { sessionExplicitlySet: boolean },
) {
  ...
  const sessionTarget = ctx.sessionExplicitlySet ? sessionTargetRaw || "" : inferredSessionTarget;
  ...
  const accountId =
    typeof opts.account === "string" && opts.account.trim() ? opts.account.trim() : undefined;
